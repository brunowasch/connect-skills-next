...
export function NotificationDropdown({
  ...
  const handleNotificationClick = (
    ...
    if (
      ...
      setFeedbackModal({
        ...
        feedback: {
          status:
            notification.feedbackData?.status ||
            (notification.type === "feedback_approved"
              ? "APPROVED"
              : "REJECTED"),
          justification:
            notification.feedbackData?.justification || notification.message,
  ...
  const getNotificationTitle = (notification: Notification) => {
    if (
      ...
      (t("notifications.types", { returnObjects: true }) as any)
    ) {
  ...
  const getNotificationMessage = (notification: Notification) => {
    ...
    if (
      notification.type === "feedback_approved" &&
      notification.message === defaultMessagesPT["feedback_approved"]
    ) {
    ...
    if (
      notification.type === "feedback_rejected" &&
      notification.message === defaultMessagesPT["feedback_rejected"]
    ) {
  ...
  return (
    <>
      <div className="relative" ref={dropdownRef}>
        ...
        {/* Dropdown */}
        {isOpen && (
          <div className="fixed left-2 right-2 top-[80px] sm:absolute sm:left-auto sm:right-0 sm:top-auto sm:mt-2 w-auto sm:w-[420px] sm:max-w-[calc(100vw-2rem)] bg-white rounded-xl shadow-2xl border border-slate-200 z-50 overflow-hidden">
            {/* Header do dropdown */}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {unreadCount > 0 && (
                    <span className="text-xs text-slate-500 bg-slate-200 px-2 py-1 rounded-full">
                      {unreadCount}{" "}
                      {unreadCount === 1
                        ? t("notifications.new_one")
                        : t("notifications.new_many")}
                    </span>
                  )}
                </div>
                  ...
                  {notifications.slice(0, 5).map((notification) => (
                    ...
                    >
                      ...
                      <div className="flex items-start gap-3">
                        {/* Icon */}
                        <div
                          className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 border ${getNotificationBgColor(notification.type)}`}
                        >
